...
public class MyDictionary implements DictInterface
{
	...
	// DictInterface class.
	public int searchPrefix(StringBuilder s,int length)
	{
		int status = 0;
	...
	public int searchPrefix(StringBuilder s, int start, int end)//considering adding the curr variable into the parameter or set the curr a null global variable and constantly change it, i think the second approach is better6
	{
		int status = 0;
		boolean doneIn, doneOut, currTest, prefix, word;
		String curr;
		...
		for (int i = 0; i < list.size() && !doneOut; i++)
		{
			...
			{
				int jj = j - start;
				//System.out.println("s index is "+j+" and jj is "+jj);
				...
				else if (s.charAt(j) < curr.charAt(jj))
				{
					doneIn = true;
					doneOut = true;
					//System.out.println("current char is "+s.charAt(j));
				}
				else 
				{
